...
const ProductList = () => {
  ...
  useEffect(()=>{
    if(!isLoading && deleteSuccess){
      toast.success('Successfully removed bro')
    }
  },[isLoading, deleteSuccess])

  if(isLoading){
    return <div className="flex justify-center w-screen mt-24"><p className="border-4 border-dashed w-32 h-32 border-green-700 rounded-full animate-spin "></p></div>
  }
